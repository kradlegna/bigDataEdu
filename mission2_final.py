...
for row in all_data:
    species = row["species"]  # 각 줄에서 species, 어떤 종류인지만 추출
...
## 2-1. 다른 방식 풀이
### 2-1-1. 정수형 변수를 만든 후 진행
"""
setosa_filt = 0
verisicolor_filt = 0
virginica_filt = 0  # 종 이름별로 개수를 세기 위한 정수형 변수 생성

for line in all_data:
    if line["species"] == "setosa": # Setosa 종류만 추출
        setosa_filt += 1
    elif line["species"] == "verisicolor": # Verisicolor 종류만 추출
        verisicolor_filt += 1
    else:
        viriginica_filt += 1 # Viriginica 종류만 추출
...
"""
species_count = {"setosa": 0, "versicolor": 0, "virginica": 0} #딕셔너리 형태로 각 종별 수를 세기 위한 변수 생성
for row in all_data:
    species = row["species"]
